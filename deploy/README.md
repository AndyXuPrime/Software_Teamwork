...
- 先做 preflight，再按需准备 `.env.local`、config renderer、`goose@v3.27.0`、
  AI Gateway seed helper、host-run 服务二进制、Knowledge runtime `.venv`/artifact 和
  Docker infra images。已存在的产物会跳过，长耗时下载/构建会持续输出进度或心跳。
