...
| Knowledge PDF E2E | local runtime stack | #440、`docs/runbooks/local-integration.md` | 手动启动 `knowledge-v2` runtime API/worker 和 host-run adapter 后上传 `DL_T_673-1999.pdf` | 覆盖真实 PDF 上传、runtime 解析、切块、embedding、索引和 `knowledge-queries` 检索；需要可用 runtime、对象存储、索引后端和 provider credential。 |
