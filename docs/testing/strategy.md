...
| 本地启动脚本 / 文档 | `bash -n scripts/local/start.sh scripts/local/stop.sh scripts/local/clean.sh scripts/local/lib/*.sh scripts/config/load-profile.sh`；`python3 scripts/verify_local_seed_contract.py`；确认 README/deploy/runbook 第一屏仍是 `cp .env.example .env.local`、`./scripts/local/start.sh`、`cd apps/web && bun install && bun run dev`，默认源保持官方，`start.sh` 先做宿主机 preflight，并只检查 `.env.local` 已存在，不创建或改写它；之后按需准备 local Go tools、`goose@v3.27.1`、服务二进制、Docker infra images、Knowledge runtime `.venv` 和 artifact；Knowledge runtime `.venv` 需要按 runtime mode 校验 dependency profile，不能只检查目录存在；`--china` 只影响本次 Go proxy、Docker registry rewrite、runtime 下载 helper 和 runtime `HF_ENDPOINT`，不改写 `config/` 或 `.env.local`；长耗时构建/下载必须持续输出进度或心跳；本地入口脚本都有开始/成功/失败命令行摘要，`start.sh` 在 migration/seed 前等待 infra health 并汇总早退服务日志，Go module proxy 排障说明区分 `GOPROXY`、Docker registry rewrite、GitHub release/raw 镜像和 `UV_DEFAULT_INDEX`，AI Gateway 本地 seed 使用 `localhost:11434`，stop 脚本按进程组停止 host-run 服务。 |
