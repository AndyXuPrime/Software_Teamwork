...
## Session 33: F-044 QA knowledge context selector

**Date**: 2026-07-04
**Task**: F-044 QA knowledge context selector
**Branch**: `Frontend/fix/qa-knowledge-context-selector`

### Summary

Prepared a QA-only PR for issue #634 with paginated knowledge-base context selection and exact ID fallback.

### Main Changes

### Summary

Created a QA-only follow-up for issue #634 after PR #649 was closed. The change adds knowledge-base context selection to QA retrieval testing and QA chat without carrying report-generation changes.

### Main Changes

- Added reusable `KnowledgeBaseMultiSelect` with server-backed pagination and exact ID fallback so users can select knowledge bases beyond the first 100 results.
- Passed selected knowledge-base IDs through QA chat streaming requests and QA retrieval-test run payloads.
- Added regression coverage for selector pagination, manual ID fallback, chat request body context IDs, and QA retrieval keyboard flow.

### Testing

- [OK] Branch created from latest `upstream/develop` @ `0f1b0331`.
- [OK] `bun run --cwd apps/web test:unit -- src/features/knowledge/components/knowledge-base-multi-select.test.tsx src/pages/qa/retrieval-test/page.a11y.test.tsx src/pages/qa/chat/page.background-stream.test.tsx src/pages/qa/chat/page.create-session.test.tsx src/api/chat.test.ts`
- [OK] `bun run --cwd apps/web check`
- [OK] `bun run --cwd apps/web test:unit`
- [OK] `bun run --cwd apps/web build`
